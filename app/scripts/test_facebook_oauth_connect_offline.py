...
so'rovi): endi reklama scope so'ralgan (ads/business tarif) HAR BIR
ulanish -- hatto bitta sahifa/hisob bo'lsa ham -- ANIQ tanlov ekraniga
(Business -> Ad Account -> Dataset) o'tkaziladi, jimgina avtomatik
saqlanmaydi. `meta_api.oauth_exchange_long_lived()` endi
`(token, expires_in)` juftini qaytaradi. Bu fayl shu ikkala o'zgarishni
aks ettiradi.
...
def test_ads_scope_always_routes_through_choose_screen_even_for_single_option():
    """2026-09 YANGI XULQ: reklama scope so'ralganda (biznes/business tarif),
    hatto bitta sahifa+hisob topilsa ham, ENDI avtomatik saqlanmaydi --
    admin Business/Ad Account/Dataset tanlov ekranini ko'rishi va yakunlashi
    SHART (foydalanuvchi talabi -- "Connect Meta -> select Business
    Portfolio -> Ad Account -> Pixel/Dataset -> Connect")."""
    ...
    try:
        with app_module.app.test_client() as client:
            ...
            assert "muvaffaqiyatli ulandi" not in html, "Reklama scope so'ralganda ENDI avtomatik saqlanmasligi kerak -- tanlov ekrani ko'rsatilishi kerak"
            assert "Qaysi hisobni ulaymiz" in html
            assert "Asosiy hisob" in html

            choose_resp = client.post("/connect-accounts/facebook/choose", data={
                "page_id": "page_1", "ad_account_id": "act_111", "dataset_id": "pixel_111",
            }, follow_redirects=True)
        ...
        meta_api.META_APP_ID, meta_api.META_APP_SECRET = "", ""
    print("OK: reklama scope so'ralganda bitta variant bo'lsa ham tanlov ekrani ko'rsatiladi, va tanlangach token shifrlangan holda + status='connected' + muddat bilan saqlanadi")


def test_ad_account_pixel_auto_detected_when_dataset_not_explicitly_chosen():
    """Agar admin tanlov formasida Dataset'ni ANIQ tanlamasa (masalan JS
    ro'yxati bo'sh chiqqan holatda) -- eski xulq saqlanadi: tanlangan
    reklama hisobiga biriktirilgan birinchi Pixel avtomatik topib
    saqlanadi (foydalanuvchi so'rovi -- "capi ni hammasini avtomatik
    qil")."""
    meta_api.META_APP_ID, meta_api.META_APP_SECRET = "test_app_id", "test_app_secret"
    meta_api.oauth_exchange_code = lambda code, redirect_uri: "short_lived_token"
    ...
    try:
        with app_module.app.test_client() as client:
            ...
            client.post("/connect-accounts/facebook/choose", data={
                "page_id": "page_pixel", "ad_account_id": "act_pixel_test",
            }, follow_redirects=True)
...
def test_business_and_dataset_selection_persisted():
    """Business Portfolio topilganda -- tanlangan Business ID/nomi va unga
    tegishli Dataset (Pixel) to'g'ri saqlanishini tekshiradi (foydalanuvchi
    talabi: "select Business Portfolio -> Ad Account -> Pixel/Dataset")."""
    meta_api.META_APP_ID, meta_api.META_APP_SECRET = "test_app_id", "test_app_secret"
    meta_api.oauth_exchange_code = lambda code, redirect_uri: "short_lived_token"
    ...
    try:
        with app_module.app.test_client() as client:
            ...
            choose_resp = client.post("/connect-accounts/facebook/choose", data={
                "page_id": "page_biz", "business_id": "biz_1",
                "ad_account_id": "act_biz_1", "dataset_id": "pixel_biz_1",
            }, follow_redirects=True)
            assert "muvaffaqiyatli ulandi" in choose_resp.get_data(as_text=True)
